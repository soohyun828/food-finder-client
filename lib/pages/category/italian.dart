...
class _ItalianPageState extends State<ItalianPage> {
      ...
      Column(
        children: [
          SizedBox(
            ...
            child: Image.asset('assets/italian.jpg', fit: BoxFit.contain),
          ),
          const SizedBox(height: 15),
          Row(
            children: [
              buildCategoryButton('파스타'),
              buildCategoryButton('피자'),
              buildCategoryButton('리조또'),
              buildCategoryButton('젤라또'),
            ],
          ),
          const SizedBox(height: 15),
          Expanded(
  ...
  Future<void> fetchRestaurants(String menu) async {
    try {
      ...
      final url = Uri.parse('$serverURL/home/italian/$menu/$username/$latitude/$longitude');
