...
class KoreanPage extends StatefulWidget {
  const KoreanPage({super.key});

  @override
  _KoreanPageState createState() => _KoreanPageState();
}
...
class _KoreanPageState extends State<KoreanPage> {
      ...
      Column(
        children: [
          SizedBox(
            ...
            child: Image.asset('assets/korean.jpg', fit: BoxFit.contain),
          ),
          const SizedBox(height: 15),
          Row(
            children: [
              buildCategoryButton('국밥'),
              buildCategoryButton('찌개'),
              buildCategoryButton('덮밥'),
              buildCategoryButton('백반'),
            ],
          ),
          const SizedBox(height: 15),
          Expanded(
  ...
  Future<void> fetchRestaurants(String menu) async {
    try {
      ...
      final url = Uri.parse('$serverURL/home/korean/$menu/$username/$latitude/$longitude');
